...
Element.Methods = {
	...
	absolutize : function(element) {
		...
		var offsets = Element.positionedOffset(element), top = offsets[1], left = offsets[0], width = element.clientWidth, height = element.clientHeight;
...
(function() {
	...
	function getPixelValue(value, property) {
		...
		if (value.include('%')) {
			...
			var whole;
			if (property.include('left') || property.include('right') || property.include('width')) {
				whole = $(element.parentNode).measure('width');
			} else if (property.include('top') || property.include('bottom') || property.include('height')) {
	...
	if ('getBoundingClientRect' in document.documentElement) {
		Element.addMethods({
			...
			positionedOffset : function(element) {
				...
				if (element.offsetParent && element.offsetParent.nodeName.toUpperCase() === 'HTML') {
